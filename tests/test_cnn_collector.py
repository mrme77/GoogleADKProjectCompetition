"""
Temporary test script to diagnose CNN collector issues.
This directly tests the CNN RSS fetching with debug output.
"""

import sys
from datetime import datetime
from manis_agent.agents.collectors.cnn_collector.tools import fetch_cnn_rss
...
def test_cnn_collector():
    print("=" * 80)
    print("CNN COLLECTOR DEBUG TEST")
    print("=" * 80)
    ...
    print("-" * 80)
    print("TESTING: CNN Politics RSS")
    print("-" * 80)
    politics_result = fetch_cnn_rss(
        category='politics',
        max_articles=3,
    ...
    # Test Tech category
    print("-" * 80)
    print("TESTING: CNN Tech RSS")
    print("-" * 80)
    tech_result = fetch_cnn_rss(
        category='tech',
        max_articles=2,
        tool_context=context
    )
    ...
    print("SUMMARY")
    print("=" * 80)
    print(f"Total CNN articles collected: {len(context.state.get('collected_articles', []))}")
    print()
